          ...
          ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
            {msg.image ? (
              ...
              <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 text-white
                 ${msg.senderId == authUser._id ? 'rounded-br-none' : 'rounded-bl-none' }`}>{msg.text}
              </p>
            )}
            {/* ID: 680f50e4f10f3cd28382ecf9 */}
            <div>
              <img src={msg.senderId === authUser._id ? 
              authUser?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.profile_martin} 
